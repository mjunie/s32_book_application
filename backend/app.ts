...
const logger = winston.createLogger({
  ...
  transports: [
    new winston.transports.File({ filename: '/var/log/books-api/error.log', level: 'error' }),
    new winston.transports.File({ filename: '/var/log/books-api/combined.log' }),
...
// ── CORS ──────────────────────────────────────────────────────────────────────
// Allow requests from your production frontend AND local dev server
app.use(cors({
  origin: [
    'https://book.s32.horizonoes.com',
    'http://localhost:4200',
  ],
  credentials: true,
}));
...
async function executeWithSpan<T>(
  ...
  return tracer.startActiveSpan(spanName, async (span: Span) => {
    ...
    try {
      ...
      bookOperationDuration.record(Date.now() - startTime, attributes || {});
      return result;
    } catch (error) {
...
// ── Protected routes (/books requires a valid Keycloak JWT) ───────────────────
...
app.post('/books', async (req: Request, res: Response) => {
  ...
  try {
    logger.info('Creating new book', { operation: 'CREATE_BOOK', title, author });

    const result = await executeWithSpan('db.insert.books', async () => {
    ...
    }, { 'db.operation': 'INSERT', 'db.table': 'books', 'db.system': 'postgresql' });
    ...
    logger.error('Error creating book', { error: error instanceof Error ? error.message : error });
...
app.get('/books', async (req: Request, res: Response) => {
  try {
    ...
    const result = await executeWithSpan('db.select.books.all', async () => {
      return await pool.query('SELECT * FROM books ORDER BY id DESC');
    }, { 'db.operation': 'SELECT', 'db.table': 'books', 'db.system': 'postgresql' });
...
app.get('/books/:id', async (req: Request, res: Response) => {
  ...
  try {
    const result = await executeWithSpan('db.select.books.by_id', async () => {
      return await pool.query('SELECT * FROM books WHERE id = $1', [id]);
    }, { 'db.operation': 'SELECT', 'db.table': 'books', 'book.id': id });
...
app.put('/books/:id', async (req: Request, res: Response) => {
  ...
  try {
    const result = await executeWithSpan('db.update.books', async () => {
      return await pool.query(
        `UPDATE books
         SET title = COALESCE($1, title), author = COALESCE($2, author),
             genre = COALESCE($3, genre), year_published = COALESCE($4, year_published),
             available_copies = COALESCE($5, available_copies)
         WHERE id = $6 RETURNING *`,
        [title, author, genre, year_published, available_copies, id]
      );
    }, { 'db.operation': 'UPDATE', 'db.table': 'books', 'book.id': id });
...
app.delete('/books/:id', async (req: Request, res: Response) => {
  ...
  try {
    const result = await executeWithSpan('db.delete.books', async () => {
      return await pool.query('DELETE FROM books WHERE id = $1 RETURNING *', [id]);
    }, { 'db.operation': 'DELETE', 'db.table': 'books', 'book.id': id });
...
app.get('/books/search/:query', async (req: Request, res: Response) => {
  ...
  try {
    const result = await executeWithSpan('db.select.books.search', async () => {
      return await pool.query(
        `SELECT * FROM books WHERE title ILIKE $1 OR author ILIKE $1 ORDER BY title`,
        [`%${query}%`]
      );
    }, { 'db.operation': 'SELECT', 'db.table': 'books', 'search.query': query });

    bookOperationCounter.add(1, { operation: 'search', status: 'success' });
    res.json({ count: result.rows.length, query, books: result.rows });
  } catch (error) {
...
app.listen(PORT, () => {
  ...
  console.log('  POST   /books               - Create a new book (🔒 auth)');
  console.log('  GET    /books               - Get all books (🔒 auth)');
  console.log('  GET    /books/:id           - Get book by ID (🔒 auth)');
  console.log('  PUT    /books/:id           - Update book (🔒 auth)');
  console.log('  DELETE /books/:id           - Delete book (🔒 auth)');
  console.log('  GET    /books/search/:query - Search books (🔒 auth)');
